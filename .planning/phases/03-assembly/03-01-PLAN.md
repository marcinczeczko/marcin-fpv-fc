...
1. `_JLCPCB_bom`: Use the built-in JLCPCB BOM template. Ensure it maps fields to JLCPCB columns (Comment, Designator, Footprint, LCSC Part #).
2. `_JLCPCB_position`: Use the built-in JLCPCB position template. Ensure it applies the `_rot_footprint_jlcpcb` filter for rotation corrections.
...
<output>
After completion, create `.planning/phases/03-assembly/03-01-SUMMARY.md`
</output>
